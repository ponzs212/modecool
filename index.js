...
function getMainKeyboard() {
  ...
  return {
    inline_keyboard: [
      [
        { text: "Bug ⸔ Menu", callback_data: "hole", style: style, icon_custom_emoji_id: "5325906128763121048" },
        { text: "Owner ⸕ Menu", callback_data: "controls", style: style, icon_custom_emoji_id: "6093786598422353987" }
      ],
      [
...
bot.start(async (ctx) => {
    ...
    const menuMessage = `<blockquote>⬡═—⊱ XILENT DEATH ⊰—═⬡
 • Wellcome To Kelap Kelip Mode. 
 • Developer : @gaponback</blockquote>
<blockquote>⬡═—⊱ INFORMATION ⊰—═⬡
 • Bot Status : Active
 • Version : 3.0 Vvip
 • Mode : Kelap Kelip
 • Type Script : Vvip Mode</blockquote>
<blockquote>⬡═—⊱ STATUS BOT ⊰—═⬡
 • Memory : ${memoryStatus}
 • Runtime : ${run}</blockquote>\n<blockquote>⬡═—⊱ STATUS SENDER ⊰—═⬡
 • Koneksi : ${senderStatus}</blockquote>`;
...
[
Markup.button.callback("❌ Batalkan Pergantian Mode", "mode_cancel")
]
])
);
});
...
await ctx.answerCbQuery();
await ctx.editMessageText("❌ Pergantian mode dibatalkan.");
});

bot.action("mode_rich", async (ctx) => {
await ctx.answerCbQuery();
await ctx.editMessageText("❌ Gagal Mengganti Mode, Mode Rich Sedang Dalam Masalah.");
});
...
async function edit(percent, status) {
    const bar = "█".repeat(percent / 10) +
                "░".repeat(10 - percent / 10);
    ...
    blocks: [
      {
        ...
        text: "👋 Selamat Datang, — 6GaponZ Mulia 🎓!"
      },
      {
        type: "RichBlockParagraph",
        text: "Bot enkripsi JavaScript milik @thisiscreeds — proteksi kode JS kamu dari reverse engineering."
      },
      {
        type: "RichBlockTable",
        rows: [
          ...
          },
          {
            cells: [
              ...
              { type: "RichBlockTableCell", text: { text: "🔴 Belum Join" } }
            ]
          },
          {
            cells: [
              { type: "RichBlockTableCell", text: { text: "Akses Enkripsi" } },
              { type: "RichBlockTableCell", text: { text: "🔒 Terkunci" } }
            ]
          }
        ]
      },
      {
        ...
        text: "© thisiscreeds — JS Obfuscation Bot"
      }
    ]
  });
});
